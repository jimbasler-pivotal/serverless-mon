...
```
tanzu apps workload create serverless-mon -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/serverless-mon-source \
  --type web
```
...
```
tanzu apps workload get serverless-mon
```

You can assign the URL to a variable to make it easier to reference it:

```
URL=$(kubectl get services.serving.knative.dev/serverless-mon -o=jsonpath='{.status.url}')
 ```
